...
// ==============================================================
`timescale 1ns/1ps
module dac_table_axilite_control_s_axi
#(parameter
    ...
    output wire [14:0]                   length_V,
    output wire [0:0]                    tlast,
    output wire [14:0]                   replay_length_V,
    output wire [0:0]                    run
);
//------------------------Address Info-------------------
...
localparam
    ...
    ADDR_IER                    = 21'h000008,
    ADDR_ISR                    = 21'h00000c,
    ADDR_LENGTH_V_DATA_0        = 21'h110000,
    ADDR_LENGTH_V_CTRL          = 21'h110004,
    ADDR_TLAST_DATA_0           = 21'h110008,
    ADDR_TLAST_CTRL             = 21'h11000c,
    ADDR_REPLAY_LENGTH_V_DATA_0 = 21'h110010,
    ADDR_REPLAY_LENGTH_V_CTRL   = 21'h110014,
    ADDR_RUN_DATA_0             = 21'h110018,
    ADDR_RUN_CTRL               = 21'h11001c,
...
//------------------------Local signal-------------------
    ...
    reg  [14:0]                   int_length_V = 'b0;
    reg  [0:0]                    int_tlast = 'b0;
    reg  [14:0]                   int_replay_length_V = 'b0;
    reg  [0:0]                    int_run = 'b0;
    // memory signals
...
//------------------------Instantiation------------------
// int_comb_0_0
dac_table_axilite_control_s_axi_ram #(
    .BYTES    ( 4 ),
    .DEPTH    ( 16384 )
) int_comb_0_0 (
    .clk0     ( ACLK ),
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_LENGTH_V_DATA_0: begin
                    rdata <= int_length_V[14:0];
                end
                ADDR_TLAST_DATA_0: begin
                    rdata <= int_tlast[0:0];
                end
                ADDR_REPLAY_LENGTH_V_DATA_0: begin
                    rdata <= int_replay_length_V[14:0];
                end
...
assign replay_length_V = int_replay_length_V;
assign run             = int_run;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_LENGTH_V_DATA_0)
            int_length_V[14:0] <= (WDATA[31:0] & wmask) | (int_length_V[14:0] & ~wmask);
    end
end
...
// int_replay_length_V[14:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_replay_length_V[14:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_REPLAY_LENGTH_V_DATA_0)
            int_replay_length_V[14:0] <= (WDATA[31:0] & wmask) | (int_replay_length_V[14:0] & ~wmask);
    end
end
...
module dac_table_axilite_control_s_axi_ram
#(parameter
    BYTES  = 4,
    DEPTH  = 256,
    AWIDTH = log2(DEPTH)
