...
#(parameter
    ...
    output wire [63:0]                   a,
    output wire [14:0]                   length_r,
    output wire [0:0]                    tlast,
    output wire [14:0]                   replay_length,
    output wire [0:0]                    run,
...
localparam
    ...
    ADDR_TLAST_DATA_0         = 6'h24,
    ADDR_TLAST_CTRL           = 6'h28,
    ADDR_REPLAY_LENGTH_DATA_0 = 6'h2c,
    ADDR_REPLAY_LENGTH_CTRL   = 6'h30,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_a = 'b0;
    reg  [14:0]                   int_length_r = 'b0;
    reg  [0:0]                    int_tlast = 'b0;
    reg  [14:0]                   int_replay_length = 'b0;
    reg  [0:0]                    int_run = 'b0;
        ...
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_LENGTH_R_DATA_0: begin
                    rdata <= int_length_r[14:0];
                end
                ADDR_TLAST_DATA_0: begin
                    rdata <= int_tlast[0:0];
                end
                ADDR_REPLAY_LENGTH_DATA_0: begin
                    rdata <= int_replay_length[14:0];
                end
...
assign replay_length     = int_replay_length;
assign run               = int_run;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_LENGTH_R_DATA_0)
            int_length_r[14:0] <= (WDATA[31:0] & wmask) | (int_length_r[14:0] & ~wmask);
    end
end
...
// int_replay_length[14:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_replay_length[14:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_REPLAY_LENGTH_DATA_0)
            int_replay_length[14:0] <= (WDATA[31:0] & wmask) | (int_replay_length[14:0] & ~wmask);
    end
end
