...
entity dac_table_axim_control_s_axi is
generic (
...
end entity dac_table_axim_control_s_axi;
...
architecture behave of dac_table_axim_control_s_axi is
    ...
    constant ADDR_TLAST_DATA_0         : INTEGER := 16#24#;
    constant ADDR_TLAST_CTRL           : INTEGER := 16#28#;
    constant ADDR_REPLAY_LENGTH_DATA_0 : INTEGER := 16#2c#;
    constant ADDR_REPLAY_LENGTH_CTRL   : INTEGER := 16#30#;
    constant ADDR_RUN_DATA_0           : INTEGER := 16#34#;
    constant ADDR_RUN_CTRL             : INTEGER := 16#38#;
    ...
    signal int_length_r        : UNSIGNED(14 downto 0) := (others => '0');
    signal int_tlast           : UNSIGNED(0 downto 0) := (others => '0');
    signal int_replay_length   : UNSIGNED(14 downto 0) := (others => '0');
            ...
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_LENGTH_R_DATA_0 =>
                        rdata_data <= RESIZE(int_length_r(14 downto 0), 32);
                    when ADDR_TLAST_DATA_0 =>
                        rdata_data <= RESIZE(int_tlast(0 downto 0), 32);
                    when ADDR_REPLAY_LENGTH_DATA_0 =>
                        rdata_data <= RESIZE(int_replay_length(14 downto 0), 32);
...
-- ----------------------- Register logic ----------------
    ...
    replay_length        <= STD_LOGIC_VECTOR(int_replay_length);
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_LENGTH_R_DATA_0) then
                    int_length_r(14 downto 0) <= (UNSIGNED(WDATA(14 downto 0)) and wmask(14 downto 0)) or ((not wmask(14 downto 0)) and int_length_r(14 downto 0));
                end if;
            end if;
        end if;
    end process;
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_REPLAY_LENGTH_DATA_0) then
                    int_replay_length(14 downto 0) <= (UNSIGNED(WDATA(14 downto 0)) and wmask(14 downto 0)) or ((not wmask(14 downto 0)) and int_replay_length(14 downto 0));
                end if;
            end if;
        end if;
    end process;
