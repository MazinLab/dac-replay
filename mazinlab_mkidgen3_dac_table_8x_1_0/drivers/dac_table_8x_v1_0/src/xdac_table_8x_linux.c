...
typedef struct {
    ...
    XDac_table_8x_uio_map maps[ MAX_UIO_MAPS ];
} XDac_table_8x_uio_info;

/***************** Variable Definitions **************************************/
static XDac_table_8x_uio_info uio_info;
...
static int uio_info_read_name(XDac_table_8x_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XDac_table_8x_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XDac_table_8x_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XDac_table_8x_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XDac_table_8x_Initialize(XDac_table_8x *InstancePtr, const char* InstanceName) {
	XDac_table_8x_uio_info *InfoPtr = &uio_info;
